...
if ! python3 work/market-data/fetch_market_data.py --market us_stock; then
  python3 work/market-data/publish_site.py
  echo '{"status":"skipped","market":"美股","reason":"行情刷新失败，已保留上一份有效数据，本轮不执行收盘复盘。"}'
  exit 2
fi
python3 work/market-data/build_price_history.py
python3 work/market-data/simulate_execution.py --market us_stock
python3 work/market-data/review_candidates.py --market us_stock
